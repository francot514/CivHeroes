...
bool DataManager::LoadStrings(const char* file) {
	...
	while(ftell(fp) < fsize) {
		...
		} else if(!strcmp(strbuf, "briefing")) {
			sscanf(&linebuf[7], "%d %99[^\n]", &value, strbuf);
			int len = BufferIO::DecodeUTF8(strbuf, strBuffer);
			wchar_t* pbuf = new wchar_t[len + 1];
			wcscpy(pbuf, strBuffer);
			_briefStrings[value] = pbuf;
...
const wchar_t* DataManager::GetBriefingString(int code) {
	if(code < 0 || code >= 2048 || _briefStrings[code] == 0)
		return unknown_string;
	return _briefStrings[code];
}
