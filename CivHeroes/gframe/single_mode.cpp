...
#include "game.h"
#include "../scriptcore/duel.h"
#include "../scriptcore/field.h"
#include "../scriptcore/mtrandom.h"
...
int SingleMode::SinglePlayThread(void* param) {
	const wchar_t* name = mainGame->lstCPlayList->getListItem(mainGame->lstCPlayList->getSelected());
	wchar_t fname[256];
	if (mainGame->cRaceC->getSelected() == 0)
		{
		  
			mainGame->dInfo.isCampaign = false;
			
			switch (mainGame->gameConf.langid)
			 {

					case 0:
						myswprintf(fname, L"./scripts/levels/english/tutorial/%ls", name);
				 break;
					case 1:
						myswprintf(fname, L"./scripts/levels/french/tutorial/%ls", name);
				 break;
					case 2:
						myswprintf(fname, L"./scripts/levels/german/tutorial/%ls", name);
				 break;
					case 3:
						myswprintf(fname, L"./scripts/levels/italian/tutorial/%ls", name);
				 break;
					case 4:
						myswprintf(fname, L"./scripts/levels/portugese/tutorial/%ls", name);
				 break;
					case 5:
						myswprintf(fname, L"./scripts/levels/spanish/tutorial/%ls", name);
				 break;

             }
         }
	else if (mainGame->cRaceC->getSelected() == 1)
		{
			mainGame->dInfo.isCampaign = true;

			switch (mainGame->gameConf.langid)
			 {

					case 0:
						myswprintf(fname, L"./scripts/levels/english/campaign/%ls", name);
				 break;
					case 1:
						myswprintf(fname, L"./scripts/levels/french/campaign/%ls", name);
				 break;
					case 2:
						myswprintf(fname, L"./scripts/levels/german/campaign/%ls", name);
				 break;
					case 3:
						myswprintf(fname, L"./scripts/levels/italian/campaign/%ls", name);
				 break;
					case 4:
						myswprintf(fname, L"./scripts/levels/portugese/campaign/%ls", name);
				 break;
					case 5:
						myswprintf(fname, L"./scripts/levels/spanish/campaign/%ls", name);
				 break;

             }
		}

	else if (mainGame->cRaceC->getSelected() == 2)
		{
			mainGame->dInfo.isCampaign = false;
			myswprintf(fname, L"./scripts/levels/skirmish/%ls", name);
	...
	set_player_info(pduel, 0, 80, 5, 1);
	set_player_info(pduel, 1, 80, 5, 1);
	mainGame->dInfo.lp[0] = 80;
	mainGame->dInfo.lp[1] = 80;
...
bool SingleMode::SinglePlayAnalyze(char* msg, unsigned int len) {
	...
	while (pbuf - msg < (int)len) {
		...
		case MSG_WIN: {
			pbuf += 2;
			DuelClient::ClientAnalyze(offset, pbuf - offset);
			if (mainGame->dInfo.isCampaign)
				{
			mainGame->gameConf.progress += 1;
