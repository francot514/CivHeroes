...
void Game::DrawMisc() {
	...
	if((dInfo.turn % 2 && dInfo.isFirst) || (!(dInfo.turn % 2) && !dInfo.isFirst)) {
		...
		driver->draw2DRectangleOutline(mainGame->Resize(327, 8, 630, 51), 0xa0000000);
	} else {
		driver->draw2DRectangle(0xa0000000, mainGame->Resize(689, 8, 991, 51));
		driver->draw2DRectangleOutline(mainGame->Resize(689, 8, 991, 51), 0xa0000000);
	}
	...
	else driver->draw2DImage(imageManager.tLPBar, mainGame->Resize(335, 12, 335 + 290 * dInfo.lp[0] / 80, 28), recti(0, 0, 16, 16), 0, 0, true);
	if(dInfo.lp[1] >= 80)
		driver->draw2DImage(imageManager.tLPBar, mainGame->Resize(696, 12, 986, 28), recti(0, 0, 16, 16), 0, 0, true);
	else driver->draw2DImage(imageManager.tLPBar, mainGame->Resize(986 - 290 * dInfo.lp[1] / 80, 12, 986 , 28), recti(0, 0, 16, 16), 0, 0, true);
	if(lpframe) {
	...
	lpcFont->draw(dataManager.GetNumString(dInfo.turn), mainGame->Resize(635, 5, 687, 40), 0x800d0d0d, true, false, 0);
	ClientCard* pcard;
	for(int i = 0; i < 5; ++i) {
...
{
	driver->draw2DRectangle(position, 0x40ffffff, 0x40ffffff, 0x40000000, 0x40000000);
...
{
	font->draw(text, position, 0x00000000, false, true);
