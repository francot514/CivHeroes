#include "replay.h"
#include "../scriptcore/ocgapi.h"
#include "../scriptcore/card.h"
#include <algorithm>
...
void Replay::BeginRecord() {
	...
	recording_fp = CreateFileW(L"./replay/_LastReplay.rep", GENERIC_WRITE, 0, NULL, CREATE_ALWAYS, FILE_FLAG_WRITE_THROUGH, NULL);
	if(recording_fp == INVALID_HANDLE_VALUE)
		return;
#else
	if(is_recording)
		fclose(fp);
	fp = fopen("./replay/_LastReplay.rep", "wb");
	if(!fp)
		return;
#endif
	pdata = replay_data;
...
void Replay::SaveReplay(const wchar_t* name) {
	wchar_t fname[64];
	myswprintf(fname, L"./replay/%ls.rep", name);
#ifdef WIN32
