...
bool DeckBuilder::OnEvent(const irr::SEvent& event) {
	...
	case irr::EET_GUI_EVENT: {
		...
		case irr::gui::EGET_SCROLL_BAR_CHANGED: {
			...
			case SCROLL_SOUND: {
				mainGame->gameConf.soundvolume = (double)mainGame->scrSound->getPos();
				mainGame->engineSound->setSoundVolume(mainGame->gameConf.soundvolume);
				break;
			}
			case SCROLL_MUSIC: {
				mainGame->gameConf.musicvolume = (double)mainGame->scrMusic->getPos();
		...
		case irr::gui::EGET_COMBO_BOX_CHANGED: {
			...
			case COMBOBOX_MAINTYPE: {
				...
				case 3: {
					...
					mainGame->cbCardType2->addItem(dataManager.GetSysString(1071), TYPE_TRAP + TYPE_CONTINUOUS);
					break;
				}
		...
		case irr::EMIE_LMOUSE_LEFT_UP: {
			...
			if(!mainGame->is_siding) {
				if((hovered_pos == 1 && (draging_pointer->second.type & 0x802040)) || (hovered_pos == 2 && !(draging_pointer->second.type & 0x802040)))
					hovered_pos = 0;
				if((hovered_pos == 1 || (hovered_pos == 0 && click_pos == 1)) && deckManager.current_deck.main.size() < 30) {
					if(hovered_seq == -1)
				...
				if((hovered_pos == 1 && (draging_pointer->second.type & 0x802040)) || (hovered_pos == 2 && !(draging_pointer->second.type & 0x802040)) || hovered_pos == 4)
					hovered_pos = 0;
				if((hovered_pos == 1 || (hovered_pos == 0 && click_pos == 1)) && deckManager.current_deck.main.size() < 35) {
					if(hovered_seq == -1)
						deckManager.current_deck.main.push_back(draging_pointer);
					else if(hovered_seq < (int)deckManager.current_deck.main.size() && hovered_pos)
						deckManager.current_deck.main.insert(deckManager.current_deck.main.begin() + hovered_seq, draging_pointer);
		...
		case irr::EMIE_RMOUSE_LEFT_UP: {
			if(mainGame->is_siding) {
				...
				if(hovered_pos == 1) {
					...
					if(!(draging_pointer->second.type & 0x802040) && deckManager.current_deck.main.size() < 34) {
						deckManager.current_deck.side.erase(deckManager.current_deck.side.begin() + hovered_seq);
						deckManager.current_deck.main.push_back(draging_pointer);
					}
				}
			...
			if(hovered_pos == 1) {
				...
				if(!is_draging)
					...
					if((draging_pointer->second.type & 0x802040) && deckManager.current_deck.extra.size() < 15) {
						deckManager.current_deck.extra.push_back(draging_pointer);
						is_draging = false;
					} else if(!(draging_pointer->second.type & 0x802040) && deckManager.current_deck.main.size() < 30) {
						deckManager.current_deck.main.push_back(draging_pointer);
						is_draging = false;
					}
				}
